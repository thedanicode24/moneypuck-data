...
```{r}
icetime_score <- ggplot(df, aes(x = situation, y = icetime)) +
  geom_boxplot(fill="darkslategray4", color="darkslategray3") +
  labs(title = "Icetime Distribution by Situation",
       x = "Situation",
       y = "Icetime") +
  theme(plot.title = element_text(face="bold")) + 
  stat_summary(fun=mean, geom="point", shape=20, 
...
```{r}
library(reshape2)
cor_df <- df %>% select(I_F_goals, I_F_points, icetime, gameScore)
correlations <- cor(cor_df)
cor_table <- melt(correlations)

p_corr <- ggplot(data=cor_table, aes(x=Var1, y=Var2, fill=value)) +
  geom_tile() +
  scale_fill_gradient2(midpoint=0, mid="grey84", limits=c(-1,1)) +
  labs(title="Correlation Matrix",
       subtitle="Correlation coefficients between game score and other variables",
       x="", y="", fill="Correlation\nCoefficient") +
  theme(plot.title=element_text(face="bold"),
        legend.title=element_text(face="bold", color="brown", size=10)) +
  geom_text(aes(label=round(value,2)), color="black", fontface="bold", size=5)
...
for (s in situations) {
  subset_df = subset(df, situation==s)
  pdf <- ggplot(subset_df, aes(x=gameScore)) +
  geom_density(alpha=.3, fill="salmon") +
  geom_vline(aes(xintercept=mean(gameScore, na.rm=TRUE)),
             color="red", linetype="longdash", linewidth=.8) +
  geom_vline(aes(xintercept=median(gameScore, na.rm=TRUE)),
             color="blue", linetype="longdash", linewidth=.8) +
  labs(title="Game Score Distribution", 
       subtitle=paste("Situation: ",s),
       x="Game Score",
       y="Density") +
  scale_x_continuous(labels=comma) +
  theme(plot.title=element_text(face="bold")) +
  annotate("text", x=mean(subset_df$gameScore)+0.1, y=0.3, 
           label="Mean", color="black", size=4, fontface="bold", angle=90) +
  annotate("text", x=median(subset_df$gameScore)-0.1, y=0.3, 
           label="Median", color="black", size=4, fontface="bold", angle=90)
  print(pdf)
}
```
...
for (s in situations) {
  subset_df = subset(df, situation==s)
  corr <- ggplot(subset_df, aes(x=icetime, y=gameScore)) +
    geom_point(size=1) +
    labs(title="Icetime vs Game Score",
         subtitle=paste("Distribution:",s), 
         x="Icetime",
         y="Game Score") +
